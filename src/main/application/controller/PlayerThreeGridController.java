...
@Component
public class PlayerThreeGridController implements GridController{
	 ...
	 * "-fx-background-color: #DE1B1B;"; private static final String
	 * YELLOW_BACKGROUND = "-fx-background-color: #F9EF1B;";
	 */
	...
	public void onTreeInsert(ActionEvent e) {
		PlayerStrategyHolder player = playerPool.getStrategyForPlayer(choiceBox3.getValue());
		if (player == null) {
			LOGGER.warning("No player found");
			return;
		}
		TreeItem<TreeObject> selectedItem = treeView3.getSelectionModel().getSelectedItem();
	...
	public void onSelectionChanged(Number oldSelection, Number newSelection) {
		String oldPlayerSelected = ChoiceSelectionHelper.getPlayerForIndex(choiceBox3, oldSelection);
		if (StringUtils.isNotEmpty(oldPlayerSelected)) {
			treeStorage.addTreeForPlayer(oldPlayerSelected, treeView3.getRoot().getChildren());
			TreeViewHelper.clearTree(treeView3);
		}

		String newPlayerSelected = ChoiceSelectionHelper.getPlayerForIndex(choiceBox3, newSelection);
		...
		if (tree != null && tree.size()>0) {
			TreeViewHelper.addItemToRoot(treeView3, tree);
		}
	...
	public void addPlayerToChoiceList(Set<String> players) {
		this.choiceBox3.getItems().clear();
		this.choiceBox3.getItems().addAll(players);
	}
